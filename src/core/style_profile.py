    ...
    *,
    ...
    async with get_session() as session:
        ...
        if contact is not None:
            contact.style_profile = profile  # JSON column accepts dict directly
            contact.style_updated_at = datetime.utcnow()

    return profile


def style_profile_as_prompt_hint(profile_data: dict | str | None) -> str:
    if not profile_data:
        return ""
    
    # Если пришла строка (например, из старого кэша или legacy кода), парсим её
