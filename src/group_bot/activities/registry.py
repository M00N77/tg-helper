...
class PulsePoll:
    """Псевдонимный пульс-опрос настроения по шкале 1..5 (проективная диагностика).

    Вопрос-метафора выбирается случайно из набора, чтобы опрос не приедался.
    Ответы псевдонимны (HMAC-хеш без прямого user_id): считаем средний балл и
    распределение, не раскрывая, кто как проголосовал.
    """
