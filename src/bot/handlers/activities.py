"""Групповые HR-активности: управление пульс-опросами и приём анонимных голосов.
...
конфликтовать со стендап-хендлером и обеспечить анонимность: для анонимной сессии
...
from src.group_bot.activities.registry import CALLBACK_PREFIX, get_activity
...
async def cmd_pulse_results(message: Message) -> None:
    ...
    async with get_session() as session:
        ...
        await close_activity_session(session, act.id)

    plugin = get_activity(act.activity_code)
    if plugin is None:
        await message.answer("❌ Неизвестная механика опроса.")
        return
    ...
    await message.answer(plugin.summarize(values, texts), parse_mode="HTML")
